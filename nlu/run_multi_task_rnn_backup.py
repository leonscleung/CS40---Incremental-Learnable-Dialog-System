...
# tf.app.flags.DEFINE_string("data_dir", "/Users/xuechunding/PycharmProjects/rnn-nlu 3/data/ATIS_samples", "Data directory")
tf.app.flags.DEFINE_string("model_dir", training_path, "Training directory.")
# tf.app.flags.DEFINE_string("model_dir", "/Users/xuechunding/PycharmProjects/rnn-nlu 3/model_tmp", "Training directory.")
...
def run_valid_test(session, model_test, mode='Test', data_set=None): # mode: Eval, Test
# Run evals on development/test set and print the accuracy.
    ...
    for bucket_id in xrange(len(_buckets)):
      ...
      for i in xrange(len(data_set[bucket_id])):
        ...
        class_logits = []
        if task['joint'] == 1:
          step_outputs = model_test.joint_step(session,
                                               encoder_inputs,
                                               tags,
                                               tag_weights,
                                               labels,
          ...
          step_outputs = model_test.tagging_step(session,
                                                 encoder_inputs,
                                                 tags,
                                                 tag_weights,
                                                 sequence_length,
          ...
          step_outputs = model_test.classification_step(session,
                                                        encoder_inputs,
                                                        labels,
                                                        sequence_length,
                                                        bucket_id,
...
class Model(object):
    def __init__(self):
        ...
        self.dataset = list(data_utils.prepare_multi_task_data(FLAGS.data_dir, FLAGS.in_vocab_size, FLAGS.out_vocab_size))
        self.pred = run_valid_test(self.session, self.model_test, mode='Test', data_set=self.dataset)
