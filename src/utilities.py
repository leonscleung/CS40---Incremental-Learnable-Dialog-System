...
import os
import re
from src.constants import STAN_JAR_PATH, STAN_MODEL_PATH
...
class PrepUtility:
    ...
    @classmethod
    def create_test_seq_in(cls, message):
        # decapitalize
        message = message.lower()
        # replace contractions:
        message = cls.handle_contractions(message)
        # remove symbols
    ...
    @classmethod
    def prepareNLUMessage(cls, message, test_seq_in_path):
        new_message = "BOS " + message + " EOS"
        count = 0
        if gfile.Exists(test_seq_in_path):
            # remove file if exists
            gfile.Remove(test_seq_in_path)
        with gfile.GFile(test_seq_in_path, mode="w") as test_seq_in:
            while count < 5:
                test_seq_in.write(new_message+"\n")
                count += 1
...
def write_json(data, file_name):
    with open(file_name, "w") as handler:
        return handler.write(json.dumps(data))

def extract_person_name(sentence):

    ner_tagger = StanfordNERTagger(STAN_MODEL_PATH, STAN_JAR_PATH, encoding='utf8')
